...
function createNoiseLayer(context, startTime, frequency, options = {}) {
  ...
  for (let index = 0; index < sampleCount; index += 1) {
    const attack = options.attack ? Math.min(1, index / (context.sampleRate * options.attack)) : 1;
    samples[index] = (2 * Math.random() - 1) * Math.exp(-index / decay) * attack;
  }
...
const effects = {
  letterImpact: (context, time, intensity = 1) => {
    const oscillator = context.createOscillator();
    const gain = context.createGain();
    oscillator.type = 'sine';
    oscillator.frequency.setValueAtTime(150, time);
    oscillator.frequency.exponentialRampToValueAtTime(55, time + 0.16);
    gain.gain.setValueAtTime(0.001, time);
    gain.gain.linearRampToValueAtTime(0.28 * intensity, time + 0.012);
    gain.gain.exponentialRampToValueAtTime(0.001, time + 0.24);
    oscillator.connect(gain).connect(context.destination);
    oscillator.start(time);
    oscillator.stop(time + 0.25);
    oscillator.onended = () => { oscillator.disconnect(); gain.disconnect(); };
    createNoiseLayer(context, time, 450, { duration: 0.055, decay: 500, filterType: 'lowpass', filterQ: 0.6, gain: 0.2 * intensity });
  },
  paperRustle: (context, time, intensity = 1) => {
    createNoiseLayer(context, time, 850, { duration: 0.24, attack: 0.025, decay: 2600, filterType: 'lowpass', filterQ: 0.5, gain: 0.09 * intensity });
  },
